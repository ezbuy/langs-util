...
const doPackLangs = (output, baseDir = process_1.cwd(), langs = defaultLangs) => {
    ...
    poFiles.forEach((path) => {
        const outDirPath = path_1.join(path_1.resolve(output), path_1.dirname(path).split(path_1.sep).reverse().join("."));
        if (!outDirAlreadyCreated[outDirPath]) {
            outDirAlreadyCreated[outDirPath] = true;
            mkdirp_1.sync(outDirPath);
        }
        copySingleFile(path_1.join(baseDir, path), path_1.join(outDirPath, path_1.basename(path)));
    });
};
    ...
    dirs.forEach((dirName) => {
        ...
        if (poFiles.length > 0) {
            ...
            poFiles.forEach((filename) => {
                ...
                console.log(`[move] ${colors_1.green(path_1.join(dirName, filename))} to ${colors_1.green(destFilename)}`);
            });
        }
    });
};
